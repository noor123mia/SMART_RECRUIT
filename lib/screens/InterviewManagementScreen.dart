...
import 'package:http/http.dart' as http; // NEW: For Zoom API
...
// Updated function (BasicAuth hata diya)
  ...
  Future<void> _scheduleInterview() async {
    ...
    try {
      ...
      // NEW: Fetch candidate email from profile
      String candidateEmail = '';
      if (_selectedCandidateId != null) {
        final prof = await _fs
            ...
            .get();
        final p = prof.data();
        candidateEmail = (p?['email'] ?? '').toString();
        if (candidateEmail.isEmpty) {
          candidateEmail = 'candidate@example.com'; // Fallback—update in DB
        }
      }
      // NEW: Combine date + time for startTime
      final timeParts = _startTimeCtrl.text.split(' ');
      final time24 = timeParts[0]; // e.g., '10:00'
      final amPm = timeParts[1]; // 'AM' or 'PM'
        ...
        switch (chosenPlatform) {
          case 'Smart Recruit Meet':
            ...
            link = _generateJitsiLink(
              ...
              start: startTime, // Use combined time
            );
            break;
          case 'Zoom':
            // NEW: Create shared Zoom meeting
            link = await _createZoomMeeting(
                  topic:
                      'Interview: ${_selectedJobTitle ?? 'Position'} - ${_selectedInterviewType ?? 'Interview'}',
                  startTime: startTime,
                  durationMins: int.tryParse(_durationCtrl.text) ?? 45,
                  candidateEmail:
            ...
            break;
          // Other platforms (Google/Teams) ke liye future mein add karo, jaise pehle bataya
          case 'Google Meet':
          case 'Microsoft Teams':
          default:
            // For now, fallback to start URL (update later with APIs)
      ...
      await _fs.collection('ScheduledInterviews').add({
        ...
        'date': DateFormat('yyyy-MM-dd').format(startTime), // Updated with time
        'time': _startTimeCtrl.text,
        'duration': int.tryParse(_durationCtrl.text) ?? 45,
        'platform': chosenPlatform,
        'meetingLink': link, // Now shared for Zoom!
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Interview scheduled with shared $chosenPlatform link: $link'),
...
import 'package:url_launcher/url_launcher.dart';
import 'package:http/http.dart' as http; // NEW: For Zoom API
// NEW: Google Meet Imports (Fixed)
import 'package:google_sign_in/google_sign_in.dart';
import 'package:googleapis/calendar/v3.dart'
    as gcal; // For http.Client // For http.Client
// NEW: Zoom Constants (Replace with your values; use env in production)
const String zoomAccountId =
    'dqZnFr_rT325_yqL4xTvBw'; // e.g., 'aBcDeFgHiJkLmNoP'
const String zoomClientId = 'MLcZrfN4TPKnbTL1h2qtSA'; // From App Credentials
const String zoomClientSecret =
    'm1QmL0cXIsiPUKNZRdv3tcVdJNuoGtNS'; // From App Credentials
// NEW: Google Meet Constants (Replace with your values from Google Cloud Console)
const String googleClientId =
    '548567710035-33k5gti2lsoe2r3ni41g2irnnalvtkrf.apps.googleusercontent.com'; // Full Client ID from Credentials
const String googleClientSecret = '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';
...
} // Regenerate and copy from Credentials
class InterviewManagementScreen extends StatefulWidget {
  const InterviewManagementScreen({Key? key}) : super(key: key);
  @override
  State<InterviewManagementScreen> createState() =>
...
// Fixed Google API Helper (INSIDE STATE CLASS)
...
// Fixed Google Meet Event Creation with CORRECT timezone handling
Future<String?> _createGoogleMeetEvent({
  ...
  // CRITICAL FIX: Google Calendar requires RFC3339 format with timezone offset
  // Pakistan is UTC+5, so add 5 hours offset manually
 
  final localStartTime = startTime;
  final localEndTime = localStartTime.add(Duration(minutes: durationMins));
 
  // Convert to RFC3339 format with +05:00 offset (Pakistan Time)
  ...
  print('Start: $startTimeStr (PKT with offset)');
  print('End: $endTimeStr (PKT with offset)');
 
  // Create event with RFC3339 formatted strings
  final event = gcal.Event(
    ...
    attendees: [
      gcal.EventAttendee(email: recruiterEmail),
      gcal.EventAttendee(email: candidateEmail),
    ],
  ...
  try {
    ...
    final meetLink = createdEvent.hangoutLink;
    print('Meet Link Created: $meetLink');
    print('Event Start: ${createdEvent.start?.dateTime}');
    print('Event End: ${createdEvent.end?.dateTime}');
    return meetLink;
  } catch (e) {
  ...
  Future<void> _scheduleInterview() async {
    ...
    try {
      ...
      if (_autoGenerateLink) {
        switch (chosenPlatform) {
          case 'Smart Recruit Meet':
            ...
            break;
          case 'Google Meet':
            // NEW: Create shared Google Meet event
            link = await _createGoogleMeetEvent(
          ...
          case 'Zoom':
            ...
            break;
          case 'Microsoft Teams':
            // For now, fallback to start URL (add API later)
            final (_, web) = _platformStartUrls(chosenPlatform);
            link = web;
            break;
          default:
      ...
      await _fs.collection('ScheduledInterviews').add({
        ...
        'duration': int.tryParse(_durationCtrl.text) ?? 45,
        'platform': chosenPlatform,
        'meetingLink': link, // Now shared for Google Meet and Zoom!
        'autoGenerated': _autoGenerateLink,
        'status': 'Scheduled',
        'recruiterId': _recruiterId,
        'createdAt': FieldValue.serverTimestamp(),
...
import 'dart:io'; // For http.Client
